...
def calculate_bias_scores(metrics: dict, intro_scores: dict | None = None, history: list[dict] | None = None) -> dict:
    ...
    for label in CORE_BIASES:
        ...
        if has_intro and has_gameplay:
            ...
            combined[label] = None
    return {"scores": combined, "intro_scores": intro, "gameplay_scores": gameplay, "evidence_counts": evidence, "weights": {"intro": 0.3, "gameplay": 0.7}}
...
def generate_final_report(data: dict) -> dict:
    ...
    if dominant:
        fallback_summary = f"{len(history)} oyun kararın ve başlangıç eğilimlerin birlikte değerlendirildi. En görünür örüntü {dominant_name} oldu; bu bir tanı değil, oyun içindeki kararlarının eğitsel bir özetidir."
        strengths = ["Karar geçmişini karşılaştırılabilir bir veri haline getirdin.", "Davranış eğilimlerini fark etmek için yeterli örnek oluşturdun."]
        growth_areas = [f"{dominant_name} ortaya çıktığında karar gerekçeni bir cümleyle kaydet.", "Karar öncesi planınla karar sonrası sonucu ayrı ayrı değerlendir."]
    else:
        fallback_summary = "Davranışsal bir eğilim belirlemek için yeterli başlangıç veya oyun kararı verisi bulunmuyor. Daha fazla karar oluştuğunda rapor anlamlı bir karşılaştırma sunabilir."
    ...
    llm_result = metin_uret(
        "Davranışsal finans eğitim raporu yaz. Tanı koyma, yatırım tavsiyesi verme, kesin al-sat dili kullanma. En fazla 90 kelime yaz.",
        f"Profil: {profile_name}\nKarar sayısı: {len(history)}\nSkorlar: {scores}\nKaynaklar:\n{kaynak_baglamini_olustur(sources)}\nTaslak: {fallback_summary}",
    ) if dominant else {"status": "not_requested", "text": None, "llm_enabled": gemini_hazir_mi(), "error_type": None}
    ...
    return {
        "agent": "final_report_agent", "title": "Davranışsal Finans Değerlendirmesi",
        "profile_type": profile.get("profile_type"), "profile_name": profile_name,
        "decision_count": len(history), "dominant_bias": dominant,
        "dominant_bias_name_tr": dominant_name, "bias_scores": scores, "bias_details": analysis,
        "summary": summary, "strengths": strengths, "growth_areas": growth_areas,
        "sources": sources, **llm_metadata,
        "agent_memory": agent_memory,
        "final_state": final_state,
        "disclaimer": "Bu değerlendirme yatırım tavsiyesi veya psikolojik tanı değildir; yalnızca oyun içindeki davranış örüntülerini açıklar.",
    }
