...
export default function YasamStandartlari({ secimler, onSecimDegis, portfoy, dolarKuru = 40, yasamGideri = 0, oturulanEvVarMi = false }) {
          ...
          <section className="bg-surface-container border border-outline card-shadow p-stack-md flex flex-col" key={kategoriId}>
            ...
            <div className="flex flex-col gap-2 mt-2">
              {kategori.secenekler.map(secenek => {
                ...
                const kilitli = kendiEviSecenegi
                  ? !oturulanEvVarMi
                  : (kategoriId === "konut" && oturulanEvVarMi)
                    ...
                    <div className="flex flex-col">
                      ...
                      {kilitli && (
                        <span className="text-error font-data-sm text-data-sm uppercase mt-1">
                          {kendiEviSecenegi
                            ? "GEREKSİNİM: BİR EVİNDE YAŞAMAYA BAŞLA"
                            : "GEREKSİNİM: ÖNCE KENDİ EVİNDEN ÇIKIŞ YAP"}
                        </span>
                      )}
